...
erDiagram
    ...
    saved_maps ||--o{ fishing_spots : "точки района, каскадом"
...
### Точка принадлежит району

`fishing_spots.mapId` — ссылка на `saved_maps` с `ON DELETE CASCADE`.

Раньше ссылки не было: точку относили к району по геометрии, «попадает в
границы — значит его». Отсюда две беды. Точка удалённого района оставалась в
базе навсегда — невидимая, потому что её район исчез, и неудаляемая, потому что
до неё было не добраться. А точка на стыке двух районов принадлежала обоим
сразу, и в обоих участвовала в расчёте.

Удаление района уносит его точки: место без района — это координаты без воды,
погоды и расчёта. Уловы при этом остаются: `catches.spotId` обнуляется
(`SET_NULL`), трофей переживает удаление места.

Миграция 25 → 26 заполняет столбец той же геометрией, по которой раньше и
считали, — ни одна существующая точка района не теряет. Когда границы
накладываются, выигрывает **меньший** район: он точнее описывает место. Точки,
не попавшие никуда, остаются без района, а не привязываются наугад: догадка
здесь хуже пустоты.
