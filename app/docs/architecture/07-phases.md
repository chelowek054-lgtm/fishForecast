...
Ни одна из фаз не начата. После 29.08.2026 работа шла внутри клиента, вне этих
фаз: модель клёва, сборы на выезд, уведомления —
[client/bite-score.md](client/bite-score.md), [client/trip-plan.md](client/trip-plan.md),
[client/alerts.md](client/alerts.md).

| Фаза | Что есть в коде |
|---|---|
| 6. Модульность | Один модуль `:app`. AGP уже 8.13, Kotlin 2.0.21 и Compose BOM 2024.10 не обновлены |
| 7. Модель клёва как данные | Веса — константы в `CalculateFishActivityUseCase`, у улова версий документов нет; задача T-0001 в очереди |
| 8. Личность и синхронизация | `uid` есть у точки ради пакета района, у улова нет; очереди исходящего нет |
| 9. Сервер и админка | Сервера в репозитории нет |
| 10. Соцчасть | Ничего |
| 11. Батиметрия и калибровка | Мель и яма вводятся руками у карты; промеров и калибровки нет |
