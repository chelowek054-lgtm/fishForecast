...
| Ход воды | Условие | 0.05 | до 6 ч того же света | Фактора нет в списке |
| Сезон | Ограничитель | — | трое суток хода воды | Фактора нет в списке |
...
Температура берётся из модели воды нужного слоя плюс самая сильная поправка
структур места (донный ключ холодит его на 3 °C; поправки не складываются — см.
«Место»). Воды нет — берётся воздух, и фактор так и
...
приток +0.5 мг/л, гнилой ил −1.5, — но не складываются: берутся лучшая
прибавка и худшая убыль.
...
```
лучшее = наибольший бонус среди структур места, если он выше нуля
худшее = наименьший бонус, если он ниже нуля
бонус  = clamp(1 + лучшее + худшее, 0.7, 1.3)
```

Бонус берётся у гильдии: коряжник хищнику +0.25, мирной рыбе +0.1; заводь
наоборот; гнилой ил обоим −0.3.

Структуры **не складываются**. Точка — одно место, и отмеченные у неё
коряжник, бровка и свал говорят об одном: рыбе здесь есть где стоять. Второй
довод «за» не делает место вдвое рыбнее, а сумма делала: пять отметок давали
×1.6, и балл средней погоды уходил под потолок от числа галочек. Поэтому в
расчёт идут самая сильная структура за рыбу и самая сильная против — коряжник
на гнилом иле остаётся коряжником на гнилом иле: 1 + 0.25 − 0.3 = 0.95.

Пределы 0.7–1.3: место сдвигает шанс не больше чем на треть в любую сторону.
Самое гиблое не обнуляет его, самое рыбное не заменяет собой погоду. Точка с
одной структурой считается как раньше: бонус любой одной структуры в словаре
не выходит за эти пределы.

Поправки структур к воде и кислороду подчиняются тому же правилу. К воде идёт
самая сильная поправка, а не сумма: два родника не холодят место на шесть
градусов. К кислороду — лучшая прибавка и худшая убыль: приток вместе с
наветренным берегом дают +0.5 мг/л, а не +0.9. Повтор одной структуры в точке
считается один раз.
...
Тот же почасовой расчёт, свёрнутый до четырёх клеток в сутки на семь дней.
Живёт в [`domain/bite/WeekActivity.kt`](../../../src/main/java/com/example/fishforecast/domain/bite/WeekActivity.kt).
...
Зори внутрь частей попадают, своей клетки не получают: шесть колонок таблица
не выдерживает, а решение принимают всё равно по частям суток — «в ночь», «на
утреннюю зорьку», «после работы».

В клетке — **среднее** по части: рыболов проводит там все шесть часов, а не
один. Час пика хранится рядом и подписывается под лучшей частью дня: ровные
шестьдесят и провал с коротким всплеском дают одинаковое среднее, но ехать за
ними надо по-разному. Подписывать пик у всех четырёх частей значило бы вернуть
на экран те же двадцать четыре часа, от которых уходили.

Прошедшие части не показываются вовсе — планировать вчерашнее утро незачем, а
пустая клетка честнее нарисованной задним числом. Неполная часть помнит, сколько
часов в неё попало.

Почасовой график отвечает «ехать ли сегодня», таблица — «когда брать отгул».
Расчёт у них один и тот же: таблица собирается из уже посчитанного ряда и не
стоит ни одного лишнего прогона модели.

### Сезон — ограничитель

Множитель по фазе вида: оцепенение 0.15, нерест 0.45, после нереста 0.65,
зимнее стояние 0.85, жара и лето 1.0, осенний нагул 1.15, преднерестовый жор
1.20. Фаза считается по воде и её ходу за трое суток — подробно в
[trip-plan.md](trip-plan.md).

Температуру фактор не повторяет: за неё отвечает свой ограничитель, и сложение
было бы двойным счётом. Сезон отвечает за то, чем рыба занята помимо еды.
Считается один раз на весь прогноз: за вечер рыба не отнерестится.

### Ход воды — вес 0.05

Окно **до 6 часов назад, но не дальше границы света**, шаг 0.3 °C. Знак зависит
не от направления, а от того, с какой стороны от оптимума стоит вода.

Граница света — час, на котором приход солнечной радиации переходит через
1 Вт/м². До неё и после неё вода идёт в разные стороны, и складывать их в одно
число нельзя: шесть часов, отсчитанные назад от полуночи, захватывали дневной
прогрев, и ночью выходило «перегретая греется дальше». Про сумму это была
правда, про ночь — нет. Если целых часов того же света набирается меньше одного,
фактор молчит.
...
### Как читается пояснение

У каждого фактора один формат: **сначала измеренное значение с единицей, потом
« — » и что это значит для этого вида**, с его порогом цифрой. Имя фактора
внутри пояснения не повторяется — оно уже стоит слева от двоеточия.

```
Температура воды: 29 °C — теплее оптимума 20–28°, привыкла к тёплой воде
Кислород:         6.4 мг/л — выше комфорта вида 5.0, для этой рыбы вода тёплая
Давление:         746 мм рт. ст. — ниже нормы 758 на 12, вид терпит 7
Тенденция:        3 мм за 14 ч — уходит вниз от нормы
Ход за сутки:     −5 мм за 24 ч — падает перед фронтом, рыба кормится впрок
Ход воды:         +0.3 °C за 6 ч — перегретая греется дальше
Ветер:            Ю 2.5 м/с — держит сторону, корм к наветренному берегу
Свет:             ночь — вид в это время стоит
Расслоение:       мель 29°, яма 17° — вода расслоилась, рыба ушла с мели вниз
Замечено:         всплески и паника малька — ещё в силе, около 1 ч
Место:            мысок — коряжник, место за рыбу
```

Правило не косметическое. Пока пороги вида не названы цифрой, вердикт выглядит
взятым с потолка: «холоднее оптимума» ничего не говорит, «29 °C — теплее
оптимума 20–28°» проверяется взглядом. Формат стережёт
`FactorCommentFormatTest`: разделитель, отсутствие повтора имени, значение
первым и никаких «+0».
...
| Ход воды | до 6 ч, обрывается на границе света | Вода инертна: за три часа её движение не отличить от шума. Дальше границы света считать нельзя — там другой ход |
...
| **Недели и месяц** | Накопленное тепло: сумма градусо-дней определяет нерест, посленерестовый провал, осенний жор | ⚠️ фаза сезона по воде и её ходу за трое суток; градусо-дней нет |
...
8. Зори и смена света → окна клёва. ✅ фазы света от восхода и заката, вес
   0.30.
...
Список сверен с кодом 12.09.2026: двенадцать факторов расчёта плюс то, что правит
их на входе.
...
6. **Место**: слой, структуры точки и их поправки к воде и кислороду — самая
   сильная за рыбу и против, без сложения, в пределах 0.7–1.3.
7. **Замечено**: отметки рыболова с затуханием к концу своего срока.
8. **Сезон** — ограничитель по фазе вида: вода и её ход за трое суток, от
   оцепенения 0.15 до преднерестового жора 1.20.
9. **Давление** — отклонение от нормы места, вес 0.35, допуск видовой и
   несимметричный.
10. **Тенденция** — окно вида (1–24 ч), вес 0.20 с множителем по времени
    отыгрыша, с различением «к норме» и «от неё».
11. **Ход давления за сутки** — вес 0.15, окно 24 часа с минимумом в 6.
12. **Свет** — фазы от восхода и заката, профиль вида или гильдии, вес 0.30.
13. **Ветер** — вес 0.15: скорость с оптимумом ряби, постоянство и разворот за
    6 часов, северный сектор со знаком по температуре воды.
14. **Ход воды** за 6 часов относительно оптимума вида, вес 0.05. Окно обрывается на рассвете и закате.
15. **Нормировка весов** по посчитанным факторам — нехватка данных не роняет
    оценку.
16. **Причина у каждого фактора** текстом.
...
2. ~~Сезон~~ — сделано 06.09.2026: фаза вида по воде и её ходу за трое суток
   входит в расчёт ограничителем. Не учтены градусо-дни и посленерестовый жор:
   истории семь суток, а восстановление длится до четырнадцати.
...
Короткий разбор с проверкой по коду и порядком работ — ниже, в разделе
...
| Градусо-дни и посленерестовый жор | Истории воды семь суток, а восстановление после нереста — до четырнадцати; сам сезон считается по воде, не по календарю |
...
| Тенденция давления вперёд | Возврат к норме = рост активности | Прогноз | ✅ (следует из часов) | ✅ «падает на N за 6 ч» | ✅ `pressureTrendFactor`, окно вида 1–24 ч |
...
| Восход и закат | Зори — главные окна клёва | ✅ `daily=sunrise,sunset` | ✅ `daily_sun` | ✅ карточка | ✅ `lightFactor`, фазы света |
...
| Сохранённые точки | Личное знание рельефа вместо карты глубин | ✅ `fishing_spots`: берег или вода, структуры, вид; точка принадлежит району, норма давления — у района |
...
| **Объём прикормки от активности** | Пассивной рыбе много корма во вред | ✅ аппетит — вода, фаза сезона и балл клёва — урезает стол, вверх не растёт ([trip-plan.md](trip-plan.md)) |
...
| **Горизонт ловли** | Дно или толща — следует из температуры и кислорода | ✅ от обычного горизонта вида: тёплая вода, нехватка кислорода и сумерки поднимают на ступень ([trip-plan.md](trip-plan.md)) |
...
| **Ход воды относительно оптимума** | до 6 ч того же света | В холодной воде прогрев в плюс, в перегретой — наоборот | Череда тёплых дней, поднявшая воду на пару градусов, запускает первое устойчивое кормление после зимы; резкое падение температуры, наоборот, останавливает питание |
...
| Точное время и балл часа | ✅ по нажатию на столбик; нажатие ловит вся колонка, а не узкий столбик |
| Когда брать отгул | ✅ неделя по частям суток под графиком: средний балл и лучший час части |
...
Что из показанного не входит в оценку — в разделе «Чего нет и почему».
