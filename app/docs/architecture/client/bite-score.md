...
| Место: слой и структуры точки | `PlaceContext` | Нет: по умолчанию мель без особенностей |
...
| Тенденция давления | 3 ч | Классическое окно барометра: короче — шум, длиннее — уже не «сейчас» |
...
| Штиль для расслоения | 6 ч подряд | Меньше — столб не успевает разделиться |
...
| Расслоение | Ограничитель | — | 6 ч | Фактора нет в списке |
| Место | Ограничитель | — | — | Бонус 1.0 |
| Давление | Условие | 0.35 | час | Фактор равен 1.0 |
| Тенденция | Условие | 0.20 | 3 ч | Фактор равен 1.0 |
...
Условие: вода выше оптимума вида **и** шесть часов подряд ветер слабее 1.5 м/с.
Тогда столб распадается на слои, рыба уходит из верхнего, но не идёт в яму, где
нечем дышать.
...
При ветре фактора нет вовсе: перемешанной воде расслаиваться нечем.
...
```
значение = falloff(|P − норма|, 12 мм рт. ст.)
```

Отклонение до 2 мм считается нормой. Нормы нет — фактор равен единице и честно
об этом пишет: подставлять сюда диапазон давления вида нельзя, норма это
свойство места.

### Тенденция за 3 часа — вес 0.20
...
Карп, вода 19 °C, кислород 8.3 мг/л, давление 753 при норме 758, за 3 часа
стабильно, за сутки −1 мм, ветер юго-западный 4.0 м/с шестой час, вечер.
...
Условия:
  давление     falloff(|753−758|, 12) = 0.58 × 0.35
  тенденция    стабильно, не у нормы = 0.80 × 0.20
  ход за сутки −1 мм → 0.90            × 0.15
  свет         вечер, профиль карпа 0.90 × 0.30
  ветер        1.0 × 1.1 (держит сторону) = 1.0 × 0.15
  ход воды     ниже оптимума, греется = 1.0 × 0.05
  итого        (0.203+0.16+0.135+0.27+0.15+0.05) / 1.20 = 0.807

Оценка: 0.93 × 0.807 × 100 ≈ 75
```
...
| **3–14 суток** | Акклимация. Рыба перестраивает обмен под температуру, в которой стоит: акклимированная к 25 °C ест почти вдвое интенсивнее акклимированной к 15 °C. Сдвиг начинается уже за сутки и в основном укладывается в первую неделю | ❌ **не учитывается** |
...
Как это встроить, когда дойдут руки: средняя вода за семь суток — это
температура акклимации; эффективный оптимум вида сдвигается к ней на ограниченную
величину (порядка ±2 °C), и текущая температура сравнивается уже со сдвинутым
оптимумом. Тогда двенадцать градусов после недели восьмиградусной воды читаются
как «потеплело, рыба вышла», а те же двенадцать после недели восемнадцати — как
«холодный удар». Сейчас модель в обоих случаях говорит одно и то же.
...
| Структуры точки в оценке места | `PlaceContext.structures` заполняется пустым списком: экраны передают только слой |
...
| Акклимация к температуре последней недели | Окно определено, механизм описан выше, но не реализован |
