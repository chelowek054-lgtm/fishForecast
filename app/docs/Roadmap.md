...
*   [ ] **Шаг 2:** Менеджер офлайн-областей (скачивание тайлов в кэш, прогресс, удаление).
*   [ ] **Шаг 3:** Слой "Секретные точки" с привязкой к истории уловов.

> ℹ️ Шаг 1 работает на демо-стиле MapLibre (`demotiles.maplibre.org`): он отдаёт
> только контуры материков, поэтому на крупном зуме карта пустая. Реальный
> источник тайлов выбирается в шаге 2 вместе с офлайн-кэшем.
...
---
**Текущий статус:** ✅ Фаза 2 закрыта: прогноз обновляется в фоне, барометр уточняет его локально.
**Следующий фокус:** Офлайн-карты (MapLibre/OSM) — Фаза 3.
