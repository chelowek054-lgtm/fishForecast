...
data class FishEntity(
    ...
    /**
     * Нерестовая полоса, °C: в ней рыбе не до еды.
     *
     * Полоса, а не точка: нерест растянут, и вид входит в него не при одном
     * значении термометра, а на прогреве через несколько градусов. Пусто —
     * про нерест вида ничего не известно, и фаза о нём молчит.
     */
    val spawnTempMinC: Float? = null,
    val spawnTempMaxC: Float? = null,
    /**
     * Ниже этой воды вид почти не кормится, °C.
     *
     * Отличается от [optMinTemp]: оптимум — где рыбе лучше всего, а это
     * граница, за которой еда перестаёт её интересовать вовсе.
     */
    val feedStartC: Float? = null,
    /**
     * Выше этой воды вид впадает в оцепенение, °C.
     *
     * Есть только у налима: он холодноводный, и летом верхняя граница решает
     * его судьбу так же, как зимняя нижняя решает судьбу карпа.
     */
    val dormantAboveC: Float? = null,
    /** Сколько суток вид отходит после нереста, прежде чем начать кормиться. */
